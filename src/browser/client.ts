/// <reference lib="dom" />
import {Client, SyncFn} from "../client";
...
export class BrowserClient {
    public constructor(public readonly client: Client, private readonly crypto: BrowserCrypto) {
    }
    ...
    public async create(key: CryptoKey, info: Json, block: Uint8Array): Promise<StackInfo> {
        ...
        const infoUi8 = BrowserCrypto.encodeEncryptedBytes(infoEnc);

        const blockEnc = await this.crypto.encryptBytes(infoKey, block);
        const blockUi8 = BrowserCrypto.encodeEncryptedBytes(blockEnc);
    ...
    public async sync(key: CryptoKey, stack: StackInfo, fn: SyncFn): Promise<StackInfo> {
        ...
        const res = await this.client.sync(stack.stack, {
            version: async (stack, info, block) => {
                ...
                const infoEnc = BrowserCrypto.decodeEncryptedBytes(infoUi8);
                ...
                const blockEnc = BrowserCrypto.decodeEncryptedBytes(blockUi8);
